...
export function SellClient({ products }: { products: ProductVM[] }) {
  ...
  const addToCart = (product: ProductVM) => {
    setCart((prev) => {
      ...
      return [...prev, { productId: product.id, quantity: 1, unitPrice: product.sellingPrice }];
    });
  };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  outOfStock ? 'bg-muted' : 'bg-primary/10 group-hover:bg-primary/15'
                )}
              >
                <Package
                  className={cn(
        ...
        <SheetContent className="flex w-full flex-col p-0 sm:max-w-md">
                ...
                return (
                  <div key={item.productId} className="flex items-center gap-3 rounded-xl border border-border p-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="w-6 text-center font-display text-base font-bold">{item.quantity}</span>
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => updateQty(item.productId, 1)}>
          ...
          {cart.length > 0 && (
            <SheetFooter className="border-t border-border p-4">
              ...
              <Button className="w-full gap-2" size="lg" onClick={completeSale} disabled={pending}>
      ...
      <Sheet open={successOpen} onOpenChange={setSuccessOpen}>
        <SheetContent side="bottom" className="flex flex-col items-center justify-center gap-4 p-8">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Product inventory has been updated</p>
          </div>
          <Button className="w-full" size="lg" onClick={() => setSuccessOpen(false)}>
            Done
          </Button>
        </SheetContent>
      </Sheet>
